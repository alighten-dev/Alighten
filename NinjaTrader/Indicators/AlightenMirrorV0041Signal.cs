/* Version ID: 2026-08-29a AlightenMirrorV0041Signal — RESET. An exact copy of
   ...
   "Clean Signal"), and its own diagnostic log names (MirrorV0041SignalLevels.log,
   MirrorZonesV0041Signal_*.log) so it can sit on the same chart as V0041 without either
...
namespace NinjaTrader.NinjaScript.Indicators
{
    ...
    public class AlightenMirrorV0041Signal : Indicator
    {
        ...
        [Display(Name = "Write Level Log", Description = "Append every level created and removed to MirrorV0041SignalLevels.log in the NinjaTrader 8 folder, for diffing against MirrorPtJV0007Signals.log. Diagnostic — leave off in normal use. Capped at 60,000 lines.", Order = 1, GroupName = "13. Diagnostics")]
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Name = "AlightenMirrorV0041Signal";
                ...
                // AlightenMirrorV0041Signal_Default.xml): A/G/H/F show Daily/240m/60m only;
                ...
                if (DebugLevelLog)
                {
                    try
                    {
                        _lvlLogPath = System.IO.Path.Combine(
                            NinjaTrader.Core.Globals.UserDataDir, "MirrorV0041SignalLevels.log");
                        _lvlLogLines = 0;
                        System.IO.File.AppendAllText(_lvlLogPath,
        ...
        private void BuildZoneSets()
        {
            ...
            NinjaTrader.Code.Output.Process(string.Format(
                "[AlightenMirrorV0041Signal] zone sets: PRIMARY {0} ({1})   INSIDE {2} ({3})",
            ...
            try
            {
                if (!System.IO.File.Exists(path))
                {
                    System.IO.File.WriteAllText(path,
                        "# AlightenMirrorV0041Signal signal groups\r\n" +
                        ...
                        "J15S, J30S, J60S; 100T\r\n" +
                        "J15L, J30L, J60L; 100T\r\n");
                    Print("[AlightenMirrorV0041Signal] created groups file with examples: " + path);
                }
                ...
                foreach (string raw in System.IO.File.ReadAllLines(path))
                {
                    ...
                    if (segs.Length < 2)
                    {
                        Print("[AlightenMirrorV0041Signal] groups file: missing '; <ticks>T' in row: " + line);
                        continue;
                    }
                    ...
                    {
                        Print("[AlightenMirrorV0041Signal] groups file: bad tick spread in row: " + line);
                        continue;
                    }
                    ...
                    for (int s = 2; s < segs.Length; s++)
                    {
                        ...
                        if (flag == "ANCHORED")
                            ...
                            Print("[AlightenMirrorV0041Signal] groups file: unknown flag '" + flag + "' in row: " + line);
                    }
                    ...
                    foreach (string tokRaw in segs[0].Split(','))
                    {
                        ...
                        {
                            Print("[AlightenMirrorV0041Signal] groups file: bad signal token '" + tok + "' in row: " + line);
                            ok = false;
                            break;
                        }
                ...
                Print("[AlightenMirrorV0041Signal] loaded " + zs.Rules.Count + " signal-group rule(s) from " + path);

                zs.LogPath = System.IO.Path.Combine(NinjaTrader.Core.Globals.UserDataDir,
                    "MirrorZonesV0041Signal_" + zs.Name + ".log");
                GrpLog(zs, "======== LOAD " + (Instrument != null ? Instrument.FullName : "?")
                    + " rules=" + zs.Rules.Count + " ========");
            }
            catch (Exception ex)
            {
                Print("[AlightenMirrorV0041Signal] groups file error: " + ex.Message);
            }
        }
        ...
        private const string TB_SETTINGS_ID = "AlightenMirrorV0041SignalSettingsBtn";
        private const string TB_EXPORT_ID   = "AlightenMirrorV0041SignalExportBtn";
        private const string TB_CLEAN_ID    = "AlightenMirrorV0041SignalCleanBtn";
        ...
        private void RemoveOrphanToolbarButtons()
        {
            ...
            try
            {
                ...
                if (doomed.Count > 0)
                    Print("[AlightenMirrorV0041Signal] cleared " + doomed.Count
                        + " orphaned toolbar button(s) left by a previous build");
            }
            catch (Exception ex) { Print("[AlightenMirrorV0041Signal] orphan sweep: " + ex.Message); }
        }
		...
		private void CreateToolbarButton()
        {
            try {
                ...
                settingsButton.ToolTip = "Configure Mirror V0041Signal visibility settings";
                ...
                exportButton.ToolTip = "Export tracked levels to CSV (V0041Signal)";
            ...
            } catch (Exception ex) { Print("[AlightenMirrorV0041Signal] toolbar error: " + ex.Message); }
        }
        ...
        private void ExportLevelsToCsv()
        {
            ...
            {
                Print("[AlightenMirrorV0041Signal] Export error: " + ex.Message);
            }
        }
        ...
        private void WriteExportMeta(string metaPath, int levelRows, int barRows)
        {
            ...
            sb.AppendLine("  \"exported_by\": \"AlightenMirrorV0041Signal\"");
            sb.AppendLine("}");
        ...
        public void OpenSettingsWindow()
		{
		    ...
		    {
		        Print($"[AlightenMirrorV0041Signal] Failed to open settings: {ex.Message}");
		    }
		}
...
namespace NinjaTrader.NinjaScript.Indicators
{
	public partial class Indicator : NinjaTrader.Gui.NinjaScript.IndicatorRenderBase
	{
		...
		{
			if (cacheAlightenMirrorV0041Signal != null)
				for (int idx = 0; idx < cacheAlightenMirrorV0041Signal.Length; idx++)
					if (cacheAlightenMirrorV0041Signal[idx] != null && cacheAlightenMirrorV0041Signal[idx].EnablePatternA == enablePatternA && cacheAlightenMirrorV0041Signal[idx].EnablePatternB == enablePatternB && cacheAlightenMirrorV0041Signal[idx].EnablePatternG == enablePatternG && cacheAlightenMirrorV0041Signal[idx].EnablePatternH == enablePatternH && cacheAlightenMirrorV0041Signal[idx].EnablePatternF == enablePatternF && cacheAlightenMirrorV0041Signal[idx].EnablePatternJ == enablePatternJ && cacheAlightenMirrorV0041Signal[idx].SrcBarsToProcess == srcBarsToProcess && cacheAlightenMirrorV0041Signal[idx].MirrorLookbackBars == mirrorLookbackBars && cacheAlightenMirrorV0041Signal[idx].EnableInvalidatedCleanup == enableInvalidatedCleanup && cacheAlightenMirrorV0041Signal[idx].ShowPatternATF1_Daily == showPatternATF1_Daily && cacheAlightenMirrorV0041Signal[idx].ShowPatternATF2_240m == showPatternATF2_240m && cacheAlightenMirrorV0041Signal[idx].ShowPatternATF3_60m == showPatternATF3_60m && cacheAlightenMirrorV0041Signal[idx].ShowPatternATF4_30m == showPatternATF4_30m && cacheAlightenMirrorV0041Signal[idx].ShowPatternATF5_15m == showPatternATF5_15m && cacheAlightenMirrorV0041Signal[idx].ShowPatternATF6_10m == showPatternATF6_10m && cacheAlightenMirrorV0041Signal[idx].ShowPatternATF7_5m == showPatternATF7_5m && cacheAlightenMirrorV0041Signal[idx].ShowPatternBTF1_Daily == showPatternBTF1_Daily && cacheAlightenMirrorV0041Signal[idx].ShowPatternBTF2_240m == showPatternBTF2_240m && cacheAlightenMirrorV0041Signal[idx].ShowPatternBTF3_60m == showPatternBTF3_60m && cacheAlightenMirrorV0041Signal[idx].ShowPatternBTF4_30m == showPatternBTF4_30m && cacheAlightenMirrorV0041Signal[idx].ShowPatternBTF5_15m == showPatternBTF5_15m && cacheAlightenMirrorV0041Signal[idx].ShowPatternBTF6_10m == showPatternBTF6_10m && cacheAlightenMirrorV0041Signal[idx].ShowPatternBTF7_5m == showPatternBTF7_5m && cacheAlightenMirrorV0041Signal[idx].ShowPatternGTF1_Daily == showPatternGTF1_Daily && cacheAlightenMirrorV0041Signal[idx].ShowPatternGTF2_240m == showPatternGTF2_240m && cacheAlightenMirrorV0041Signal[idx].ShowPatternGTF3_60m == showPatternGTF3_60m && cacheAlightenMirrorV0041Signal[idx].ShowPatternGTF4_30m == showPatternGTF4_30m && cacheAlightenMirrorV0041Signal[idx].ShowPatternGTF5_15m == showPatternGTF5_15m && cacheAlightenMirrorV0041Signal[idx].ShowPatternGTF6_10m == showPatternGTF6_10m && cacheAlightenMirrorV0041Signal[idx].ShowPatternGTF7_5m == showPatternGTF7_5m && cacheAlightenMirrorV0041Signal[idx].ShowPatternHTF1_Daily == showPatternHTF1_Daily && cacheAlightenMirrorV0041Signal[idx].ShowPatternHTF2_240m == showPatternHTF2_240m && cacheAlightenMirrorV0041Signal[idx].ShowPatternHTF3_60m == showPatternHTF3_60m && cacheAlightenMirrorV0041Signal[idx].ShowPatternHTF4_30m == showPatternHTF4_30m && cacheAlightenMirrorV0041Signal[idx].ShowPatternHTF5_15m == showPatternHTF5_15m && cacheAlightenMirrorV0041Signal[idx].ShowPatternHTF6_10m == showPatternHTF6_10m && cacheAlightenMirrorV0041Signal[idx].ShowPatternHTF7_5m == showPatternHTF7_5m && cacheAlightenMirrorV0041Signal[idx].ShowPatternFTF1_Daily == showPatternFTF1_Daily && cacheAlightenMirrorV0041Signal[idx].ShowPatternFTF2_240m == showPatternFTF2_240m && cacheAlightenMirrorV0041Signal[idx].ShowPatternFTF3_60m == showPatternFTF3_60m && cacheAlightenMirrorV0041Signal[idx].ShowPatternFTF4_30m == showPatternFTF4_30m && cacheAlightenMirrorV0041Signal[idx].ShowPatternFTF5_15m == showPatternFTF5_15m && cacheAlightenMirrorV0041Signal[idx].ShowPatternFTF6_10m == showPatternFTF6_10m && cacheAlightenMirrorV0041Signal[idx].ShowPatternFTF7_5m == showPatternFTF7_5m && cacheAlightenMirrorV0041Signal[idx].ShowPatternJTF1_Daily == showPatternJTF1_Daily && cacheAlightenMirrorV0041Signal[idx].ShowPatternJTF2_240m == showPatternJTF2_240m && cacheAlightenMirrorV0041Signal[idx].ShowPatternJTF3_60m == showPatternJTF3_60m && cacheAlightenMirrorV0041Signal[idx].ShowPatternJTF4_30m == showPatternJTF4_30m && cacheAlightenMirrorV0041Signal[idx].ShowPatternJTF5_15m == showPatternJTF5_15m && cacheAlightenMirrorV0041Signal[idx].ShowPatternJTF6_10m == showPatternJTF6_10m && cacheAlightenMirrorV0041Signal[idx].ShowPatternJTF7_5m == showPatternJTF7_5m && cacheAlightenMirrorV0041Signal[idx].ColorTF1 == colorTF1 && cacheAlightenMirrorV0041Signal[idx].ColorTF2 == colorTF2 && cacheAlightenMirrorV0041Signal[idx].ColorTF3 == colorTF3 && cacheAlightenMirrorV0041Signal[idx].ColorTF4 == colorTF4 && cacheAlightenMirrorV0041Signal[idx].ColorTF5 == colorTF5 && cacheAlightenMirrorV0041Signal[idx].ColorTF6 == colorTF6 && cacheAlightenMirrorV0041Signal[idx].ColorTF7 == colorTF7 && cacheAlightenMirrorV0041Signal[idx].LevelWidth == levelWidth && cacheAlightenMirrorV0041Signal[idx].LevelDashStyleA == levelDashStyleA && cacheAlightenMirrorV0041Signal[idx].LevelDashStyleB == levelDashStyleB && cacheAlightenMirrorV0041Signal[idx].LevelDashStyleG == levelDashStyleG && cacheAlightenMirrorV0041Signal[idx].LevelDashStyleH == levelDashStyleH && cacheAlightenMirrorV0041Signal[idx].LevelDashStyleF == levelDashStyleF && cacheAlightenMirrorV0041Signal[idx].LevelDashStyleJ == levelDashStyleJ && cacheAlightenMirrorV0041Signal[idx].ShowLevelLabels == showLevelLabels && cacheAlightenMirrorV0041Signal[idx].SyncThrottleMs == syncThrottleMs && cacheAlightenMirrorV0041Signal[idx].EnableResearchLog == enableResearchLog && cacheAlightenMirrorV0041Signal[idx].ResearchTargetTicks == researchTargetTicks && cacheAlightenMirrorV0041Signal[idx].ExportMode == exportMode && cacheAlightenMirrorV0041Signal[idx].LabelFontSize == labelFontSize && cacheAlightenMirrorV0041Signal[idx].LabelOffsetTicks == labelOffsetTicks && cacheAlightenMirrorV0041Signal[idx].EqualsInput(input))
						return cacheAlightenMirrorV0041Signal[idx];
			return CacheIndicator<AlightenMirrorV0041Signal>(new AlightenMirrorV0041Signal(){ EnablePatternA = enablePatternA, EnablePatternB = enablePatternB, EnablePatternG = enablePatternG, EnablePatternH = enablePatternH, EnablePatternF = enablePatternF, EnablePatternJ = enablePatternJ, SrcBarsToProcess = srcBarsToProcess, MirrorLookbackBars = mirrorLookbackBars, EnableInvalidatedCleanup = enableInvalidatedCleanup, ShowPatternATF1_Daily = showPatternATF1_Daily, ShowPatternATF2_240m = showPatternATF2_240m, ShowPatternATF3_60m = showPatternATF3_60m, ShowPatternATF4_30m = showPatternATF4_30m, ShowPatternATF5_15m = showPatternATF5_15m, ShowPatternATF6_10m = showPatternATF6_10m, ShowPatternATF7_5m = showPatternATF7_5m, ShowPatternBTF1_Daily = showPatternBTF1_Daily, ShowPatternBTF2_240m = showPatternBTF2_240m, ShowPatternBTF3_60m = showPatternBTF3_60m, ShowPatternBTF4_30m = showPatternBTF4_30m, ShowPatternBTF5_15m = showPatternBTF5_15m, ShowPatternBTF6_10m = showPatternBTF6_10m, ShowPatternBTF7_5m = showPatternBTF7_5m, ShowPatternGTF1_Daily = showPatternGTF1_Daily, ShowPatternGTF2_240m = showPatternGTF2_240m, ShowPatternGTF3_60m = showPatternGTF3_60m, ShowPatternGTF4_30m = showPatternGTF4_30m, ShowPatternGTF5_15m = showPatternGTF5_15m, ShowPatternGTF6_10m = showPatternGTF6_10m, ShowPatternGTF7_5m = showPatternGTF7_5m, ShowPatternHTF1_Daily = showPatternHTF1_Daily, ShowPatternHTF2_240m = showPatternHTF2_240m, ShowPatternHTF3_60m = showPatternHTF3_60m, ShowPatternHTF4_30m = showPatternHTF4_30m, ShowPatternHTF5_15m = showPatternHTF5_15m, ShowPatternHTF6_10m = showPatternHTF6_10m, ShowPatternHTF7_5m = showPatternHTF7_5m, ShowPatternFTF1_Daily = showPatternFTF1_Daily, ShowPatternFTF2_240m = showPatternFTF2_240m, ShowPatternFTF3_60m = showPatternFTF3_60m, ShowPatternFTF4_30m = showPatternFTF4_30m, ShowPatternFTF5_15m = showPatternFTF5_15m, ShowPatternFTF6_10m = showPatternFTF6_10m, ShowPatternFTF7_5m = showPatternFTF7_5m, ShowPatternJTF1_Daily = showPatternJTF1_Daily, ShowPatternJTF2_240m = showPatternJTF2_240m, ShowPatternJTF3_60m = showPatternJTF3_60m, ShowPatternJTF4_30m = showPatternJTF4_30m, ShowPatternJTF5_15m = showPatternJTF5_15m, ShowPatternJTF6_10m = showPatternJTF6_10m, ShowPatternJTF7_5m = showPatternJTF7_5m, ColorTF1 = colorTF1, ColorTF2 = colorTF2, ColorTF3 = colorTF3, ColorTF4 = colorTF4, ColorTF5 = colorTF5, ColorTF6 = colorTF6, ColorTF7 = colorTF7, LevelWidth = levelWidth, LevelDashStyleA = levelDashStyleA, LevelDashStyleB = levelDashStyleB, LevelDashStyleG = levelDashStyleG, LevelDashStyleH = levelDashStyleH, LevelDashStyleF = levelDashStyleF, LevelDashStyleJ = levelDashStyleJ, ShowLevelLabels = showLevelLabels, SyncThrottleMs = syncThrottleMs, EnableResearchLog = enableResearchLog, ResearchTargetTicks = researchTargetTicks, ExportMode = exportMode, LabelFontSize = labelFontSize, LabelOffsetTicks = labelOffsetTicks }, input, ref cacheAlightenMirrorV0041Signal);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.AlightenMirrorV0041Signal AlightenMirrorV0041Signal(bool enablePatternA, bool enablePatternB, bool enablePatternG, bool enablePatternH, bool enablePatternF, bool enablePatternJ, int srcBarsToProcess, int mirrorLookbackBars, bool enableInvalidatedCleanup, bool showPatternATF1_Daily, bool showPatternATF2_240m, bool showPatternATF3_60m, bool showPatternATF4_30m, bool showPatternATF5_15m, bool showPatternATF6_10m, bool showPatternATF7_5m, bool showPatternBTF1_Daily, bool showPatternBTF2_240m, bool showPatternBTF3_60m, bool showPatternBTF4_30m, bool showPatternBTF5_15m, bool showPatternBTF6_10m, bool showPatternBTF7_5m, bool showPatternGTF1_Daily, bool showPatternGTF2_240m, bool showPatternGTF3_60m, bool showPatternGTF4_30m, bool showPatternGTF5_15m, bool showPatternGTF6_10m, bool showPatternGTF7_5m, bool showPatternHTF1_Daily, bool showPatternHTF2_240m, bool showPatternHTF3_60m, bool showPatternHTF4_30m, bool showPatternHTF5_15m, bool showPatternHTF6_10m, bool showPatternHTF7_5m, bool showPatternFTF1_Daily, bool showPatternFTF2_240m, bool showPatternFTF3_60m, bool showPatternFTF4_30m, bool showPatternFTF5_15m, bool showPatternFTF6_10m, bool showPatternFTF7_5m, bool showPatternJTF1_Daily, bool showPatternJTF2_240m, bool showPatternJTF3_60m, bool showPatternJTF4_30m, bool showPatternJTF5_15m, bool showPatternJTF6_10m, bool showPatternJTF7_5m, Brush colorTF1, Brush colorTF2, Brush colorTF3, Brush colorTF4, Brush colorTF5, Brush colorTF6, Brush colorTF7, int levelWidth, DashStyleHelper levelDashStyleA, DashStyleHelper levelDashStyleB, DashStyleHelper levelDashStyleG, DashStyleHelper levelDashStyleH, DashStyleHelper levelDashStyleF, DashStyleHelper levelDashStyleJ, bool showLevelLabels, int syncThrottleMs, bool enableResearchLog, int researchTargetTicks, bool exportMode, int labelFontSize, int labelOffsetTicks)
		{
			return indicator.AlightenMirrorV0041Signal(Input, enablePatternA, enablePatternB, enablePatternG, enablePatternH, enablePatternF, enablePatternJ, srcBarsToProcess, mirrorLookbackBars, enableInvalidatedCleanup, showPatternATF1_Daily, showPatternATF2_240m, showPatternATF3_60m, showPatternATF4_30m, showPatternATF5_15m, showPatternATF6_10m, showPatternATF7_5m, showPatternBTF1_Daily, showPatternBTF2_240m, showPatternBTF3_60m, showPatternBTF4_30m, showPatternBTF5_15m, showPatternBTF6_10m, showPatternBTF7_5m, showPatternGTF1_Daily, showPatternGTF2_240m, showPatternGTF3_60m, showPatternGTF4_30m, showPatternGTF5_15m, showPatternGTF6_10m, showPatternGTF7_5m, showPatternHTF1_Daily, showPatternHTF2_240m, showPatternHTF3_60m, showPatternHTF4_30m, showPatternHTF5_15m, showPatternHTF6_10m, showPatternHTF7_5m, showPatternFTF1_Daily, showPatternFTF2_240m, showPatternFTF3_60m, showPatternFTF4_30m, showPatternFTF5_15m, showPatternFTF6_10m, showPatternFTF7_5m, showPatternJTF1_Daily, showPatternJTF2_240m, showPatternJTF3_60m, showPatternJTF4_30m, showPatternJTF5_15m, showPatternJTF6_10m, showPatternJTF7_5m, colorTF1, colorTF2, colorTF3, colorTF4, colorTF5, colorTF6, colorTF7, levelWidth, levelDashStyleA, levelDashStyleB, levelDashStyleG, levelDashStyleH, levelDashStyleF, levelDashStyleJ, showLevelLabels, syncThrottleMs, enableResearchLog, researchTargetTicks, exportMode, labelFontSize, labelOffsetTicks);
		}
