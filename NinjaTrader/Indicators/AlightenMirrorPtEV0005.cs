...
{
    public class AlightenMirrorPtEV0005 : Indicator
    {
		...
		private class PatternECandidate
		{
            ...
            public double StreakStartTimestamp; // NEW: Track start of consistency
		...
		[NinjaScriptProperty]
		[Display(Name="Gate: Require Pivot for Pattern E Signal", GroupName="Pattern E", Order=41)]
		public bool RequirePivotForPatternESignal { get; set; } = false;

		[NinjaScriptProperty]
        [Display(Name="Show Debug Labels", GroupName="Pattern E", Order=92)]
		...
		#region Pattern E Detailed Level Variables

		private Series<double> patternELongLevel;
        ...
        [Browsable(false)]
        [XmlIgnore()]
        public Series<double> PatternELongLevel => patternELongLevel;
        

        private Series<double> patternELongStartTimestamp;
        [Browsable(false)]
        [XmlIgnore()]
        public Series<double> PatternELongStartTimestamp => patternELongStartTimestamp;

        private Series<double> patternELongEndTimestamp;
        [Browsable(false)]
        [XmlIgnore()]
        public Series<double> PatternELongEndTimestamp => patternELongEndTimestamp;

        private Series<double> patternEShortLevel;
        [Browsable(false)]
        [XmlIgnore()]
        public Series<double> PatternEShortLevel => patternEShortLevel;

        private Series<double> patternEShortStartTimestamp;
        [Browsable(false)]
        [XmlIgnore()]
        public Series<double> PatternEShortStartTimestamp => patternEShortStartTimestamp;

        private Series<double> patternEShortEndTimestamp;
        [Browsable(false)]
        [XmlIgnore()]
        public Series<double> PatternEShortEndTimestamp => patternEShortEndTimestamp;
		
		private Series<double> patternELongSignalBarTimestamp;
		[Browsable(false)]
		[XmlIgnore()]
		public Series<double> PatternELongSignalBarTimestamp => patternELongSignalBarTimestamp;
		
		private Series<double> patternEShortSignalBarTimestamp;
		[Browsable(false)]
		[XmlIgnore()]
		public Series<double> PatternEShortSignalBarTimestamp => patternEShortSignalBarTimestamp;
        ...
        protected override void OnStateChange()
        {
            if (State == State.SetDefaults)
            {
                ...
                AddPlot(Brushes.Transparent, "PatternELongLevel");
                AddPlot(Brushes.Transparent, "PatternELongStartTimestamp");
                AddPlot(Brushes.Transparent, "PatternELongEndTimestamp");
                AddPlot(Brushes.Transparent, "PatternEShortLevel");
                AddPlot(Brushes.Transparent, "PatternEShortStartTimestamp");
                AddPlot(Brushes.Transparent, "PatternEShortEndTimestamp");
				AddPlot(Brushes.Transparent, "PatternELongSignalBarTimestamp");
				AddPlot(Brushes.Transparent, "PatternEShortSignalBarTimestamp");
            ...
            else if (State == State.DataLoaded)
            {
                ...
                patternELongLevel               = Values[2];
                patternELongStartTimestamp      = Values[3];
                patternELongEndTimestamp        = Values[4];
                patternEShortLevel              = Values[5];
                patternEShortStartTimestamp     = Values[6];
                patternEShortEndTimestamp       = Values[7];
				patternELongSignalBarTimestamp  = Values[8];
				patternEShortSignalBarTimestamp = Values[9];
        ...
        protected override void OnBarUpdate()
		{
		    ...
		    if (BarsInProgress == 0)
		    {
		        ...
		        if (EnablePatternESignal)
		        {
					...
					patternELongLevel[0]          = 0;
		            patternELongStartTimestamp[0] = 0;
		            patternELongEndTimestamp[0]   = 0;
		            
		            patternEShortLevel[0]          = 0;
		            patternEShortStartTimestamp[0] = 0;
		            patternEShortEndTimestamp[0]   = 0;
					
					patternELongSignalBarTimestamp[0]  = 0;
					patternEShortSignalBarTimestamp[0] = 0;
		...
		private void UpdatePatternE_Live_BIP0()
		{
		    ...
		    if (CurrentBar < 1)
		    {
		        ...
		        if (CanDrawDebug() && DrawPatternE)
		        {
                    ...
                    {
                        RemoveDrawObject(PE_LIVE_TAG_L + "_" + i);
                        RemoveDrawObject(PE_LIVE_TAG_S + "_" + i);
                        RemoveDrawObject(PE_LIVE_LVL_L + "_" + i);
                        RemoveDrawObject(PE_LIVE_LVL_S + "_" + i);
                    }
                    // clear legacy single tags if any
            ...
            List<PatternECandidate> liveLongCands = new List<PatternECandidate>();
            List<PatternECandidate> liveShortCands = new List<PatternECandidate>();
		    ...
		    // Single scalar liveSig for plot (tie-break if both)
		    double liveSig = 0;
            ...
            if (liveLongCands.Count > 0)
            {
                 ...
                 if (!livePreciseEndTimesL.ContainsKey(curBar))
                 {
                      ...
                      {
                           string key = c.Key + "_Historical";
                           recordedPatternKeys.Add(key);
                      }
                 }
                 
                 // PLOT CONTINUOUSLY (every tick) using latched value
                 PatternELongEndTimestamp[0]   = livePreciseEndTimesL[curBar];
				 PatternELongSignalBarTimestamp[0] = Time[0].ToOADate();
                 
                 var best = liveLongCands[0];
                 PatternELongLevel[0]          = best.OgLevel;
                 PatternELongStartTimestamp[0] = (best.OgTime != Core.Globals.MinDate) ? best.OgTime.ToOADate() : 0;
            ...
            if (liveShortCands.Count > 0)
            {
                 ...
                 PatternEShortEndTimestamp[0]   = livePreciseEndTimesS[curBar];
				 PatternEShortSignalBarTimestamp[0] = Time[0].ToOADate();
                 
                 var best = liveShortCands[0];
                 PatternEShortLevel[0]          = best.OgLevel;
                 PatternEShortStartTimestamp[0] = (best.OgTime != Core.Globals.MinDate) ? best.OgTime.ToOADate() : 0;
            }
		    ...
		    if (CanDrawDebug() && DrawPatternE)
		    {
				...
				foreach(var c in liveLongCands)
				{
				    ...
				    int startBar = c.LineStartBip0BarIdx;
				    // Safety fallback
				    if (startBar < 0) startBar = CurrentBars[0] - 1;
				    if (startBar > CurrentBars[0]) startBar = CurrentBars[0] - 1;
				    ...
				    Draw.Line(this, PE_LIVE_LVL_L + suffix, false,
				        barsAgoStart, c.OgLevel,
				        0,          c.OgLevel,
				        PatternELongColor, PatternELevelDashStyle, PatternELevelWidth);
				
				    double yL = Low[0] - (PatternEMarkerOffsetTicks * TickSize);
				    Draw.Text(this, PE_LIVE_TAG_L + suffix, false,
				        "▲",
				        0, yL, 0,
				        PatternELongColor, new SimpleFont("Arial", PatternEMarkerFontSize),
				...
				foreach(var c in liveShortCands)
				{
				    ...
				    Draw.Line(this, PE_LIVE_LVL_S + suffix, false,
				        barsAgoStart, c.OgLevel,
				        0,          c.OgLevel,
				        PatternEShortColor, PatternELevelDashStyle, PatternELevelWidth);
				
				    double yS = High[0] + (PatternEMarkerOffsetTicks * TickSize);
				    Draw.Text(this, PE_LIVE_TAG_S + suffix, false,
				        "▼",
				        0, yS, 0,
				        PatternEShortColor, new SimpleFont("Arial", PatternEMarkerFontSize),
				        TextAlignment.Center, Brushes.Transparent, Brushes.Transparent, 0);
...
//			                            c.LineStartBip0BarIdx = sigBar;
                                    ...
                                    if (EnablePatternESignal)
                                    {
                                        int barsAgo = CurrentBar - sigBar;
                                        string plotKey = c.Key + "_Historical";                                                                            
                                        ...
                                        if (!recordedPatternKeys.Contains(plotKey))
                                        {
                                            ...
                                            if (barsAgo >= 0 && barsAgo < CurrentBar)
                                            {
                                                ...
                                                if (livePreciseEndTimesL != null && livePreciseEndTimesL.ContainsKey(sigBar))
                                                    preciseTime = livePreciseEndTimesL[sigBar];
                                                else if (PatternELongEndTimestamp[barsAgo] > 0)
                                                    preciseTime = PatternELongEndTimestamp[barsAgo];
                                                    
                                                if (preciseTime == 0)
                                                    preciseTime = Times[0][barsAgo].ToOADate();
                                                    
                                                c.StreakStartTimestamp = preciseTime;
                                                
                                                PatternELongEndTimestamp[barsAgo]   = preciseTime;
                                                PatternELongStartTimestamp[barsAgo] = (c.OgTime != Core.Globals.MinDate) ? c.OgTime.ToOADate() : 0;
                                                PatternELongLevel[barsAgo]          = c.OgLevel;
												PatternELongSignalBarTimestamp[barsAgo] = Times[0][barsAgo].ToOADate();
                                            }
                                            else
                                            {
                                            }
			                ...
			                else
			                {
			                    if (isWick)
			                    {
                                    ...
                                    if (EnablePatternESignal && c.StreakStartTimestamp > 0)
                                    {
                                        int barsAgo = CurrentBar - sigBar;
                                        if (barsAgo >= 0 && barsAgo < CurrentBar)
                                        {
                                            PatternELongEndTimestamp[barsAgo]   = c.StreakStartTimestamp;
                                            PatternELongStartTimestamp[barsAgo] = (c.OgTime != Core.Globals.MinDate) ? c.OgTime.ToOADate() : 0;
                                            PatternELongLevel[barsAgo]          = c.OgLevel;
											PatternELongSignalBarTimestamp[barsAgo] = Times[0][barsAgo].ToOADate();
										}
...
//			                            c.LineStartBip0BarIdx = sigBar;
                                    ...
                                    if (EnablePatternESignal)
                                    {
                                        ...
                                        if (!recordedPatternKeys.Contains(plotKey))
                                        {
                                            ...
                                            if (barsAgo >= 0 && barsAgo < CurrentBar)
                                            {
                                                ...
                                                if (livePreciseEndTimesS != null && livePreciseEndTimesS.ContainsKey(sigBar))
                                                    preciseTime = livePreciseEndTimesS[sigBar];
                                                else if (PatternEShortEndTimestamp[barsAgo] > 0)
                                                    preciseTime = PatternEShortEndTimestamp[barsAgo];
                                                ...
                                                PatternEShortEndTimestamp[barsAgo]   = preciseTime;
                                                PatternEShortStartTimestamp[barsAgo] = (c.OgTime != Core.Globals.MinDate) ? c.OgTime.ToOADate() : 0;
                                                PatternEShortLevel[barsAgo]          = c.OgLevel;
												PatternEShortSignalBarTimestamp[barsAgo] = Times[0][barsAgo].ToOADate();
                                            }
                                            else
                                            {
                                            }
			                ...
			                else
			                {
			                    if (isWick)
			                    {
                                    ...
                                    if (EnablePatternESignal && c.StreakStartTimestamp > 0)
                                    {
                                        ...
                                        if (barsAgo >= 0 && barsAgo < CurrentBar)
                                        {
                                            PatternEShortEndTimestamp[barsAgo]   = c.StreakStartTimestamp;
                                            PatternEShortStartTimestamp[barsAgo] = (c.OgTime != Core.Globals.MinDate) ? c.OgTime.ToOADate() : 0;
                                            PatternEShortLevel[barsAgo]          = c.OgLevel;
											PatternEShortSignalBarTimestamp[barsAgo] = Times[0][barsAgo].ToOADate();
                                        }
            ...
            if (ShowDebugLabels)
            {
                 foreach (var kv in patternELongByBar)
                 {
                    ...
                    double level = PatternELongLevel[barsAgo];
                    if (level < 0.0001) continue; 

                    double startTs = PatternELongStartTimestamp[barsAgo];
                    double endTs   = PatternELongEndTimestamp[barsAgo];

                    DrawPatternEDetailLabel(true, barIndex, level, startTs, endTs);
                 }
                 ...
                 foreach (var kv in patternEShortByBar)
                 {
                    ...
                    double level = PatternEShortLevel[barsAgo];
                    if (level < 0.0001) continue; 

                    double startTs = PatternEShortStartTimestamp[barsAgo];
                    double endTs   = PatternEShortEndTimestamp[barsAgo];

                    DrawPatternEDetailLabel(false, barIndex, level, startTs, endTs);
                 }
            }

		}

		private void DrawPatternEDetailLabel(bool isLong, int barIndex, double level, double startTsOA, double endTsOA)
        {
             ...
             string tag = isLong ? $"PE_DET_L_{barIndex}" : $"PE_DET_S_{barIndex}";
             
             // Position: slightly above/below the triangle
             double y = isLong 
                ? Low[barsAgo] - (PatternEMarkerOffsetTicks + 15) * TickSize 
                : High[barsAgo] + (PatternEMarkerOffsetTicks + 15) * TickSize;

             Draw.Text(this, tag, false, text, barsAgo, y, 0,
                isLong ? PatternELongColor : PatternEShortColor,
		    ...
		    if (patternELevels != null)
		    {
		        foreach (var lvl in patternELevels)
		        {
		            string tagOld = lvl.IsLong
		                ? $"PATTERNE_LVL_L_{lvl.OgBarIndex}"
		                : $"PATTERNE_LVL_S_{lvl.OgBarIndex}";
...
//		    string tag  = isLong ? $"PATTERNE_L_{barIndex}" : $"PATTERNE_S_{barIndex}";
//		    string text = isLong ? "▲" : "▼";
...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private AlightenMirrorPtEV0005[] cacheAlightenMirrorPtEV0005;
		public AlightenMirrorPtEV0005 AlightenMirrorPtEV0005(int barsToProcess, bool enablePatternESignal, bool enablePatternELevels, bool drawPatternE, int patternEMarkerFontSize, int patternEMarkerOffsetTicks, System.Windows.Media.Brush patternELongColor, System.Windows.Media.Brush patternEShortColor, int patternELevelWidth, DashStyleHelper patternELevelDashStyle, bool requirePivotForPatternESignal, bool showDebugLabels, bool showZigZag, System.Windows.Media.Brush zigZagColor, int zigZagWidth, DashStyleHelper zigZagStyle)
		{
			return AlightenMirrorPtEV0005(Input, barsToProcess, enablePatternESignal, enablePatternELevels, drawPatternE, patternEMarkerFontSize, patternEMarkerOffsetTicks, patternELongColor, patternEShortColor, patternELevelWidth, patternELevelDashStyle, requirePivotForPatternESignal, showDebugLabels, showZigZag, zigZagColor, zigZagWidth, zigZagStyle);
		}

		public AlightenMirrorPtEV0005 AlightenMirrorPtEV0005(ISeries<double> input, int barsToProcess, bool enablePatternESignal, bool enablePatternELevels, bool drawPatternE, int patternEMarkerFontSize, int patternEMarkerOffsetTicks, System.Windows.Media.Brush patternELongColor, System.Windows.Media.Brush patternEShortColor, int patternELevelWidth, DashStyleHelper patternELevelDashStyle, bool requirePivotForPatternESignal, bool showDebugLabels, bool showZigZag, System.Windows.Media.Brush zigZagColor, int zigZagWidth, DashStyleHelper zigZagStyle)
		{
			if (cacheAlightenMirrorPtEV0005 != null)
				for (int idx = 0; idx < cacheAlightenMirrorPtEV0005.Length; idx++)
					if (cacheAlightenMirrorPtEV0005[idx] != null && cacheAlightenMirrorPtEV0005[idx].BarsToProcess == barsToProcess && cacheAlightenMirrorPtEV0005[idx].EnablePatternESignal == enablePatternESignal && cacheAlightenMirrorPtEV0005[idx].EnablePatternELevels == enablePatternELevels && cacheAlightenMirrorPtEV0005[idx].DrawPatternE == drawPatternE && cacheAlightenMirrorPtEV0005[idx].PatternEMarkerFontSize == patternEMarkerFontSize && cacheAlightenMirrorPtEV0005[idx].PatternEMarkerOffsetTicks == patternEMarkerOffsetTicks && cacheAlightenMirrorPtEV0005[idx].PatternELongColor == patternELongColor && cacheAlightenMirrorPtEV0005[idx].PatternEShortColor == patternEShortColor && cacheAlightenMirrorPtEV0005[idx].PatternELevelWidth == patternELevelWidth && cacheAlightenMirrorPtEV0005[idx].PatternELevelDashStyle == patternELevelDashStyle && cacheAlightenMirrorPtEV0005[idx].RequirePivotForPatternESignal == requirePivotForPatternESignal && cacheAlightenMirrorPtEV0005[idx].ShowDebugLabels == showDebugLabels && cacheAlightenMirrorPtEV0005[idx].ShowZigZag == showZigZag && cacheAlightenMirrorPtEV0005[idx].ZigZagColor == zigZagColor && cacheAlightenMirrorPtEV0005[idx].ZigZagWidth == zigZagWidth && cacheAlightenMirrorPtEV0005[idx].ZigZagStyle == zigZagStyle && cacheAlightenMirrorPtEV0005[idx].EqualsInput(input))
						return cacheAlightenMirrorPtEV0005[idx];
			return CacheIndicator<AlightenMirrorPtEV0005>(new AlightenMirrorPtEV0005(){ BarsToProcess = barsToProcess, EnablePatternESignal = enablePatternESignal, EnablePatternELevels = enablePatternELevels, DrawPatternE = drawPatternE, PatternEMarkerFontSize = patternEMarkerFontSize, PatternEMarkerOffsetTicks = patternEMarkerOffsetTicks, PatternELongColor = patternELongColor, PatternEShortColor = patternEShortColor, PatternELevelWidth = patternELevelWidth, PatternELevelDashStyle = patternELevelDashStyle, RequirePivotForPatternESignal = requirePivotForPatternESignal, ShowDebugLabels = showDebugLabels, ShowZigZag = showZigZag, ZigZagColor = zigZagColor, ZigZagWidth = zigZagWidth, ZigZagStyle = zigZagStyle }, input, ref cacheAlightenMirrorPtEV0005);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.AlightenMirrorPtEV0005 AlightenMirrorPtEV0005(int barsToProcess, bool enablePatternESignal, bool enablePatternELevels, bool drawPatternE, int patternEMarkerFontSize, int patternEMarkerOffsetTicks, System.Windows.Media.Brush patternELongColor, System.Windows.Media.Brush patternEShortColor, int patternELevelWidth, DashStyleHelper patternELevelDashStyle, bool requirePivotForPatternESignal, bool showDebugLabels, bool showZigZag, System.Windows.Media.Brush zigZagColor, int zigZagWidth, DashStyleHelper zigZagStyle)
		{
			return indicator.AlightenMirrorPtEV0005(Input, barsToProcess, enablePatternESignal, enablePatternELevels, drawPatternE, patternEMarkerFontSize, patternEMarkerOffsetTicks, patternELongColor, patternEShortColor, patternELevelWidth, patternELevelDashStyle, requirePivotForPatternESignal, showDebugLabels, showZigZag, zigZagColor, zigZagWidth, zigZagStyle);
		}

		public Indicators.AlightenMirrorPtEV0005 AlightenMirrorPtEV0005(ISeries<double> input , int barsToProcess, bool enablePatternESignal, bool enablePatternELevels, bool drawPatternE, int patternEMarkerFontSize, int patternEMarkerOffsetTicks, System.Windows.Media.Brush patternELongColor, System.Windows.Media.Brush patternEShortColor, int patternELevelWidth, DashStyleHelper patternELevelDashStyle, bool requirePivotForPatternESignal, bool showDebugLabels, bool showZigZag, System.Windows.Media.Brush zigZagColor, int zigZagWidth, DashStyleHelper zigZagStyle)
		{
			return indicator.AlightenMirrorPtEV0005(input, barsToProcess, enablePatternESignal, enablePatternELevels, drawPatternE, patternEMarkerFontSize, patternEMarkerOffsetTicks, patternELongColor, patternEShortColor, patternELevelWidth, patternELevelDashStyle, requirePivotForPatternESignal, showDebugLabels, showZigZag, zigZagColor, zigZagWidth, zigZagStyle);
		}
	}
