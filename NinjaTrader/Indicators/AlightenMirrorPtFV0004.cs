...
namespace NinjaTrader.NinjaScript.Indicators
{
    ...
    // Pattern F (V0004)
    //
    // V0004 (2026-09-15) — CLOSED-BAR LEVEL PUBLISHING FIX
    //   Copied from V0003. V0003 wrote patternFLongLevel[1] / patternFShortLevel[1]
    ...
    //   from patternFLevels, a path the Mirror never reads.
    //   Each direction now publishes independently. patternFSignal is unchanged and
    //   still reports 0 on an ambiguous bar.
    //
    //   Same defect and fix as PtG V0003 (diagnosed 2026-09-14 on a G15S+J15S zone
    //   at 29558.75), PtA V0011 and PtH V0003. PtB is unaffected (it never writes
    ...
    // =====================================================================
    public class AlightenMirrorPtFV0004 : Indicator
    {
		        ...
		        if (allowHeavy)
		        {
		            ...
		            if (EnablePatternFSignal)
		            {
		                ...
		                if (CurrentBars[0] >= 1)
		                {
		                    ...
		                    // ------------------------------------------------------------------
		                    // V0004 FIX (same defect fixed in PtG V0003): publish each direction's
		                    // LEVEL independently. patternFLongLevel and patternFShortLevel are
		                    ...
		                    // patternFLevels, a path the Mirror never reads.
		                    //
		                    ...
		                    // ------------------------------------------------------------------
		                    if (longNow && patternFLongLevelByBar != null
		                        && patternFLongLevelByBar.TryGetValue(sigBar, out double lvlPriceLong))
		                        patternFLongLevel[1] = lvlPriceLong;

		                    if (shortNow && patternFShortLevelByBar != null
		                        && patternFShortLevelByBar.TryGetValue(sigBar, out double lvlPriceShort))
		                        patternFShortLevel[1] = lvlPriceShort;

		                    // patternFSignal keeps its single-direction semantics UNCHANGED for
...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private AlightenMirrorPtFV0004[] cacheAlightenMirrorPtFV0004;
		public AlightenMirrorPtFV0004 AlightenMirrorPtFV0004(int barsToProcess, bool enablePatternFSignal, bool enablePatternFLevels, bool drawPatternF, int patternFMarkerFontSize, int patternFMarkerOffsetTicks, System.Windows.Media.Brush patternFLongColor, System.Windows.Media.Brush patternFShortColor, int patternFLevelWidth, DashStyleHelper patternFLevelDashStyle, bool showDebugLabels, bool showZigZag, System.Windows.Media.Brush zigZagColor, int zigZagWidth, DashStyleHelper zigZagStyle)
		{
			return AlightenMirrorPtFV0004(Input, barsToProcess, enablePatternFSignal, enablePatternFLevels, drawPatternF, patternFMarkerFontSize, patternFMarkerOffsetTicks, patternFLongColor, patternFShortColor, patternFLevelWidth, patternFLevelDashStyle, showDebugLabels, showZigZag, zigZagColor, zigZagWidth, zigZagStyle);
		}

		public AlightenMirrorPtFV0004 AlightenMirrorPtFV0004(ISeries<double> input, int barsToProcess, bool enablePatternFSignal, bool enablePatternFLevels, bool drawPatternF, int patternFMarkerFontSize, int patternFMarkerOffsetTicks, System.Windows.Media.Brush patternFLongColor, System.Windows.Media.Brush patternFShortColor, int patternFLevelWidth, DashStyleHelper patternFLevelDashStyle, bool showDebugLabels, bool showZigZag, System.Windows.Media.Brush zigZagColor, int zigZagWidth, DashStyleHelper zigZagStyle)
		{
			if (cacheAlightenMirrorPtFV0004 != null)
				for (int idx = 0; idx < cacheAlightenMirrorPtFV0004.Length; idx++)
					if (cacheAlightenMirrorPtFV0004[idx] != null && cacheAlightenMirrorPtFV0004[idx].BarsToProcess == barsToProcess && cacheAlightenMirrorPtFV0004[idx].EnablePatternFSignal == enablePatternFSignal && cacheAlightenMirrorPtFV0004[idx].EnablePatternFLevels == enablePatternFLevels && cacheAlightenMirrorPtFV0004[idx].DrawPatternF == drawPatternF && cacheAlightenMirrorPtFV0004[idx].PatternFMarkerFontSize == patternFMarkerFontSize && cacheAlightenMirrorPtFV0004[idx].PatternFMarkerOffsetTicks == patternFMarkerOffsetTicks && cacheAlightenMirrorPtFV0004[idx].PatternFLongColor == patternFLongColor && cacheAlightenMirrorPtFV0004[idx].PatternFShortColor == patternFShortColor && cacheAlightenMirrorPtFV0004[idx].PatternFLevelWidth == patternFLevelWidth && cacheAlightenMirrorPtFV0004[idx].PatternFLevelDashStyle == patternFLevelDashStyle && cacheAlightenMirrorPtFV0004[idx].ShowDebugLabels == showDebugLabels && cacheAlightenMirrorPtFV0004[idx].ShowZigZag == showZigZag && cacheAlightenMirrorPtFV0004[idx].ZigZagColor == zigZagColor && cacheAlightenMirrorPtFV0004[idx].ZigZagWidth == zigZagWidth && cacheAlightenMirrorPtFV0004[idx].ZigZagStyle == zigZagStyle && cacheAlightenMirrorPtFV0004[idx].EqualsInput(input))
						return cacheAlightenMirrorPtFV0004[idx];
			return CacheIndicator<AlightenMirrorPtFV0004>(new AlightenMirrorPtFV0004(){ BarsToProcess = barsToProcess, EnablePatternFSignal = enablePatternFSignal, EnablePatternFLevels = enablePatternFLevels, DrawPatternF = drawPatternF, PatternFMarkerFontSize = patternFMarkerFontSize, PatternFMarkerOffsetTicks = patternFMarkerOffsetTicks, PatternFLongColor = patternFLongColor, PatternFShortColor = patternFShortColor, PatternFLevelWidth = patternFLevelWidth, PatternFLevelDashStyle = patternFLevelDashStyle, ShowDebugLabels = showDebugLabels, ShowZigZag = showZigZag, ZigZagColor = zigZagColor, ZigZagWidth = zigZagWidth, ZigZagStyle = zigZagStyle }, input, ref cacheAlightenMirrorPtFV0004);
		}
	}
}
